////FETCH
const cardInfa = document.querySelector('.card-container');
...
const fetchData = async () => {
    try {
        ...
        data.forEach((post) => {
            ...
            card.classList.add('card_post');

            card.innerHTML = `
                <h2 class="card-title">${post.title}</h2>
                <p class="card-description">${post.body}</p>
                <img src="../images/meme_js.jpg" alt="Card Image" class="card-img">
            `;

            cardInfa.appendChild(card);
        });
    } catch (error) {
